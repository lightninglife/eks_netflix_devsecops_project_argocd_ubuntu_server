resource "null_resource" "get_argocd_admin_password" {
  provisioner "remote-exec" {
    inline = [
      "ssh -o StrictHostKeyChecking=no -i \"web-ec2.pem\" ubuntu@${data.aws_instance.eks_netflix.private_ip} \"${var.null_resource_get_argocd_admin_password_remote_exec_inline}\""
    ]
...
resource "null_resource" "output_argocd_admin_password" {
  provisioner "remote-exec" {
    inline = [
      ...
      "ssh -o StrictHostKeyChecking=no -i \"web-ec2.pem\" -y ubuntu@${data.aws_instance.eks_netflix.private_ip} \"${var.null_resource_output_argocd_admin_password_remote_exec_inline}\""
    ]
  connection {
