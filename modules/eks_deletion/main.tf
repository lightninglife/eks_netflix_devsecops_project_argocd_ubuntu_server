resource "null_resource" "patch_ingress" {
  provisioner "remote-exec" {
    inline = [
      "ssh -i \"web-ec2.pem\"  ubuntu@${data.aws_instance.eks_netflix.private_ip} \"${var.null_resource_patch_ingress_remote_exec_inline}" # "kubectl patch ingress argocd-server -n argocd -p '{\"metadata\":{\"finalizers\":[]}}' --type=merge"
    ]
  }
